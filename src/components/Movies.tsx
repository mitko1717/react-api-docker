      ...
      <div className="flex flex-col w-full">
        <div className="flex">
          ...
          <div className="flex h-[50%] self-center gap-x-2">
            sort by:
            <Button variant="contained" size="small" onClick={() => setSortingBy('year')}>
              <span className="text-white">{sortingBy === 'year' ? <span className="font-bold">year</span>: <span>year</span>}</span>
            </Button>
            <Button variant="contained" size="small" onClick={() => setSortingBy('title')}>
              <span className="text-white">{sortingBy === 'title' ? <span className="font-bold">title</span>: <span>title</span>}</span>
            </Button>
          </div>
        </div>
        {
          data && data.data && data.data.length ? <div className="flex flex-wrap">
          {data.data.map((movie) => {
            return (
              <Movie key={movie.id} movie={movie} actorSearch={actorSearch} />
            );
          })}
        </div> : <div className="font-bold text-center text-2xl">there is no results. add some movies or change search params</div>
        }
      </div>
    );
};
